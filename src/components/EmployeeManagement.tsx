  ...
  return (
    <div className="p-4 px-0 md:p-6">
      ...
      {showAddEmployeeModal && (
        <div className="fixed inset-0 bg-black/50 flex items-start justify-center p-4 z-50 overflow-y-auto pt-12 md:pt-20" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowAddEmployeeModal(false); }}>
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border border-white/10">
            ...
            <form onSubmit={handleAddEmployee} className="p-4 md:p-5 overflow-y-auto space-y-4">
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Thông tin cơ bản</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="col-span-1 sm:col-span-2">
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Họ Tên <span className="text-red-500">*</span></label>
                    <input type="text" required value={newEmployee.fullName} onChange={e => setNewEmployee({ ...newEmployee, fullName: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập họ và tên" />
                  </div>
                  <div className="grid grid-cols-2 gap-3 col-span-1 sm:col-span-2">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số Điện Thoại <span className="text-red-500">*</span></label>
                      <input type="tel" required value={newEmployee.phone || ''} onChange={e => setNewEmployee({ ...newEmployee, phone: e.target.value.replace(/\D/g, '') })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập SĐT" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số CCCD</label>
                      <input type="text" value={newEmployee.cccd || ''} onChange={e => setNewEmployee({ ...newEmployee, cccd: e.target.value.replace(/\D/g, '') })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập số CCCD" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Chi nhánh <span className="text-red-500">*</span></label>
                      <select value={newEmployee.locationId} onChange={e => setNewEmployee({ ...newEmployee, locationId: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
                        {getAllowedBranches().map(branch => <option key={branch} value={branch}>{branch}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số TK Ngân Hàng</label>
                      <input type="text" value={newEmployee.bankAccount || ''} onChange={e => setNewEmployee({ ...newEmployee, bankAccount: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl font-bold text-xs" placeholder="Số TK" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Thông tin bổ sung</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số Ca/Tuần</label>
                      <input type="number" value={newEmployee.shiftsPerWeek || ''} onChange={e => { let val = parseInt(e.target.value); if (isNaN(val)) val = 0; if (val > 21) val = 21; setNewEmployee({ ...newEmployee, shiftsPerWeek: val }); }} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold text-xs text-center" placeholder="Số ca" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Ngày Vào Làm</label>
                      <input type="date" value={newEmployee.joinDate || ''} onChange={e => setNewEmployee({ ...newEmployee, joinDate: e.target.value })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold text-xs" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Vị trí</label>
                      <select value={(newEmployee.defaultRole || 'PV')} onChange={e => setNewEmployee({ ...newEmployee, defaultRole: e.target.value as 'QUẦY' | 'PV' })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
                        <option value="QUẦY">QUẦY</option>
                        <option value="PV">PHỤC VỤ</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Loại hình</label>
                      <select value={(newEmployee.workType || 'Part Time')} onChange={e => setNewEmployee({ ...newEmployee, workType: e.target.value as 'Part Time' | 'Full Time' })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <div className="space-y-3">
                  <div className="col-span-1 sm:col-span-2">
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Ghi chú</label>
                    <textarea 
                      value={newEmployee.notes || ''} 
                      onChange={e => setNewEmployee({ ...newEmployee, notes: e.target.value })} 
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl font-medium text-xs focus:ring-2 focus:ring-blue-500 outline-none min-h-[70px] resize-none bg-white" 
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                <button type="button" onClick={() => setShowAddEmployeeModal(false)} className="px-5 py-2 bg-slate-100 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest">Hủy</button>
                <button type="submit" disabled={isSubmitting} className={`px-5 py-2 ${adminTheme.button} text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg ${adminTheme.shadow}`}>
      ...
      {showEditEmployeeModal && editingEmployee && (
        <div className="fixed inset-0 bg-black/50 flex items-start justify-center p-4 z-50 overflow-y-auto pt-12 md:pt-20" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowEditEmployeeModal(false); }}>
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border border-white/10">
            <div className={`flex items-center justify-between p-4 border-b border-white/10 ${adminTheme.header}`}>
            ...
            <form onSubmit={handleUpdateEmployee} className="p-4 md:p-5 overflow-y-auto space-y-4">
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Thông tin cơ bản</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="col-span-1 sm:col-span-2">
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Họ Tên <span className="text-red-500">*</span></label>
                    <input type="text" required value={editingEmployee.fullName} onChange={e => setEditingEmployee({ ...editingEmployee, fullName: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập họ và tên" />
                  </div>
                  <div className="grid grid-cols-2 gap-3 col-span-1 sm:col-span-2">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số Điện Thoại <span className="text-red-500">*</span></label>
                      <input type="tel" required value={editingEmployee.phone || ''} onChange={e => setEditingEmployee({ ...editingEmployee, phone: e.target.value.replace(/\D/g, '') })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập SĐT" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số CCCD</label>
                      <input type="text" value={editingEmployee.cccd || ''} onChange={e => setEditingEmployee({ ...editingEmployee, cccd: e.target.value.replace(/\D/g, '') })} className="w-full px-3 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-xs" placeholder="Nhập số CCCD" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Chi nhánh <span className="text-red-500">*</span></label>
                      <select value={editingEmployee.locationId} onChange={e => setEditingEmployee({ ...editingEmployee, locationId: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
                        {getAllowedBranches().map(branch => <option key={branch} value={branch}>{branch}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số TK Ngân Hàng</label>
                      <input type="text" value={editingEmployee.bankAccount || ''} onChange={e => setEditingEmployee({ ...editingEmployee, bankAccount: e.target.value })} className="w-full px-3 py-2 border border-slate-200 rounded-xl font-bold text-xs" placeholder="Số TK" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Số Ca/Tuần</label>
                      <input type="number" value={editingEmployee.shiftsPerWeek || ''} onChange={e => { let val = parseInt(e.target.value); if (isNaN(val)) val = 0; if (val > 21) val = 21; setEditingEmployee({ ...editingEmployee, shiftsPerWeek: val }); }} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold text-xs text-center" placeholder="Số ca" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Ngày Vào Làm</label>
                      <input type="date" value={editingEmployee.joinDate || ''} onChange={e => setEditingEmployee({ ...editingEmployee, joinDate: e.target.value })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold text-xs" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Vị trí</label>
                      <select value={(editingEmployee.defaultRole || 'PV')} onChange={e => setEditingEmployee({ ...editingEmployee, defaultRole: e.target.value as 'QUẦY' | 'PV' })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
                        <option value="QUẦY">QUẦY</option>
                        <option value="PV">PHỤC VỤ</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-500 uppercase mb-1 ml-1">Loại hình</label>
                      <select value={(editingEmployee.workType || 'Part Time')} onChange={e => setEditingEmployee({ ...editingEmployee, workType: e.target.value as 'Part Time' | 'Full Time' })} className="w-full px-2.5 py-2 border border-slate-200 rounded-xl font-bold bg-white text-xs">
                        <option value="Part Time">Part Time</option>
                        <option value="Full Time">Full Time</option>
                      </select>
                    </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                <button type="button" onClick={() => setShowEditEmployeeModal(false)} className="px-5 py-2 bg-slate-100 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest">Hủy</button>
                <button type="submit" disabled={isSubmitting} className={`px-5 py-2 ${adminTheme.button} text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg ${adminTheme.shadow}`}>
